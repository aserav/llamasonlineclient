...
CONFIG_FILE = "llamasonlineclientconfig.json"
...
def print_header():
    ...
    print(" Llama's Online Client - Permanent VC Presence ".center(60))
...
async def join_voice(ws, guild_id, channel_id):
    ...
    print_status(f"Joining voice: {guild_id}:{channel_id}", "system")
...
def voice_menu(last_guild, last_channel, config):
    ...
    if not asyncio.run(validate_token(token)):
        ...
        return
    if not asyncio.run(validate_channel(token, guild_id, channel_id)):
